...
public class BFS {
    ...
    private void dfs2(List<List<Integer>> adj,boolean[] isVisited, int i) {
        isVisited[i] = true;
        for(Integer node :adj.get(i)){
            if(!isVisited[node])
                dfs2(adj,isVisited,node);
        }
    }
    ...
    public boolean validTree(int n, int[][] edges) {
        ...
        parent.put(0,-1);
        return dfs1(adj,parent,0,-1) && parent.size() == n;
    }

    private boolean dfs1(List<List<Integer>> adj, Map<Integer,Integer> parent, int i,int parentIndx) {
       // parent.put(i,parentIndx);
        for(Integer node :adj.get(i)){
            if(parent.get(i).intValue() == node){
                continue;
            }
    ...
    private void bfs(char[][] grid, boolean[][] isVisited, int i, int j) {
        ...
        while(!pos.isEmpty()){
            ...
            for(int p =0 ; p < 4;p++){
                int r = cell[0]+dir[p][0];
                int c = cell[1]+dir[p][1];
                if(r < 0 || r >= grid.length || c <0 || c>=grid[0].length || isVisited[r][c] || grid[r][c] == '0'){
                    continue;
                }
                pos.offer(new int[]{r,c});
                isVisited[r][c] = true;
            }
        }
    }

    private void dfs(char[][] grid, boolean[][] isVisited, int r, int c) {
        if(r < 0 || r >= grid.length || c <0 || c>=grid[0].length || isVisited[r][c] || grid[r][c] == '0'){
            return;
        }
        isVisited[r][c] = true;
        int[][] dir = {{0,1},{0,-1},{1,0},{-1,0}};
        for(int i =0 ; i < 4;i++){
            dfs(grid,isVisited,r+ dir[i][0],c+dir[i][1]);
        }
    }

    public Node cloneGraphDFs(Node node) {
        Map<Node,Node> isVisited = new HashMap<>();
        cloneRec(isVisited,node);
        return isVisited.get(node);
    }

    private Node cloneRec(Map<Node, Node> isVisited, Node node) {
        Node newNode = new Node(node.val);
        isVisited.put(node,newNode);
        for(Node neighbour : node.neighbors){
            if(!isVisited.containsKey(neighbour)){
                cloneRec(isVisited,neighbour);
            }
            newNode.neighbors.add(isVisited.get(neighbour));
        }
        return newNode;
    }
    ...
    public Node cloneGraph(Node node) {
        ...
        isVisited.put(node,root);

        while(!oldnodes.isEmpty()) {
            Node front = oldnodes.poll();
            Node newNode = isVisited.get(front);
            //if()
            for(Node neighbour :front.neighbors){
                Node newNeighbour = isVisited.get(neighbour);
                ...
                if(!isVisited.containsKey(neighbour)) {
                    ...
                    isVisited.put(neighbour,newNeighbour);
                }
            }
        }
        return isVisited.get(node);
    ...
    int findMin(Graph g,int source,int destination){
        ...
        boolean[] isInStack = new boolean[g.vertices];
        return findMin(g,isVisited,source,destination,0,isInStack);
    }

    private int findMin(Graph g, boolean[] isVisited, int source, int destination, int count,boolean[] isInStack) {
        ...
        DoublyLinkedList.Node head = g.adjacencyList[source].headNode;
        if(head == null){
            return min;
        }
        while(head != null){
           if(!isVisited[(int)head.data]){
              min = Math.min(min,findMin(g,isVisited,source,destination,count+1,isInStack));
           }
           else if((int)head.data == destination){

           }
           head = head.nextNode;
        }
        isInStack[source] = false;
        return min;
    }

    public static boolean checkPath(Graph g, int source, int destination) {
        boolean[] isVisited = new boolean[g.vertices];
        return DFS(isVisited,g,source,destination);
    }

    private static boolean DFS(boolean[] isVisited, Graph g, int source, int destination) {
        if(source == destination){
            return true;
        }
        isVisited[source] = true;
        boolean res = false;
        DoublyLinkedList.Node head = g.adjacencyList[source].getHeadNode();
        while(head != null){
            if(!isVisited[(int)head.data]){
                res = DFS(isVisited,g,(int)head.data,destination);
                if(res){
                    return res;
                }
            }
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        LinkedList<Integer>[] adjMatrix = new LinkedList[numCourses];
        for(int i =0 ; i<numCourses;i++){
            adjMatrix[i] = new LinkedList<>();
        }
        for(int j =0 ; j<prerequisites.length;j++){
            adjMatrix[prerequisites[j][0]].add(prerequisites[j][1]);
        }
